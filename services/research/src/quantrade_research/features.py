...
BASELINE_FEATURE_DEFINITIONS: tuple[FeatureDefinition, ...] = (
    ...
    FeatureDefinition(
        ...
        required_inputs=("filing_facts:us-gaap:NetIncomeLoss", "filing_facts:dei:EntityCommonStockSharesOutstanding", "daily_price_bars:split_adjusted"),
        as_of_rule="Use only filing facts whose available_at and price session are at or before decision_at; never infer an unfiled quarter.",
    ),
    FeatureDefinition(
        ...
        required_inputs=("filing_facts:us-gaap:NetIncomeLoss", "filing_facts:us-gaap:Assets"),
        as_of_rule="Use only facts from filings available at or before decision_at, with both asset observations eligible at that time.",
    ),
    FeatureDefinition(
