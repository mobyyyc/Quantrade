...
export async function getRetrospectiveBasketPreview(scoreDate: string): Promise<PreviousPaperPortfolioResult | undefined> {
  const result = await databasePool().query(
    `WITH selected AS (
       ...
       WHERE snapshot.score_date = $1::date
     ...
     SELECT dates.start_date::text, dates.end_date::text,
     ...
     CROSS JOIN window_dates dates
     GROUP BY dates.start_date, dates.end_date, dates.benchmark_start, dates.benchmark_end`,
    [scoreDate],
  );
