...
export default async function ResearchPage() {
  ...
  return <AppShell current="/research">
    ...
    })}</ul><p className="portfolio-checkpoints-note">Each checkpoint uses the original next-open execution and its actual later market close. Missing marks or corporate actions are withheld, not estimated.</p></div></> : <p>No official monthly portfolio is active yet. The first one will be fixed from the final eligible score publication of a completed calendar month, then use the next session&apos;s regular open after its market data passes validation.</p>}</div></section>
