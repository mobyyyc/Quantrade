...
import { getLatestDatedScores, getLatestPaperPortfolio, getPreviousDatedScores, getRetrospectiveBasketPreview, listDatedScores, ResearchReadModelError, type DatedScore, type PaperPortfolio, type PreviousPaperPortfolioResult } from "@/lib/research-read-model";
...
export default async function RankingsPage({ searchParams }: { searchParams: Promise<{ date?: string; basketPreview?: string }> }) {
  ...
  let previewResult: PreviousPaperPortfolioResult | undefined;
  ...
  try {
    ...
    if (scoreDate) {
      const [previous, datedPortfolio, retrospectivePreview] = await Promise.all([
        getPreviousDatedScores(scoreDate),
        getLatestPaperPortfolio(scoreDate),
        previewEnabled ? getRetrospectiveBasketPreview(scoreDate) : Promise.resolve(undefined),
      ]);
      previousScoreDate = previous?.scoreDate;
      previousScores = previous?.scores ?? [];
      portfolio = datedPortfolio;
      previewResult = retrospectivePreview;
    }
  } catch (error) {
