...
export default async function StockDetailPage({ params, searchParams }: { params: Promise<{ securityId: string }>; searchParams: Promise<{ date?: string; from?: string }> }) {
  ...
  const requiredInputsByModel: Record<string, number> = {
    tier_b_regularized_linear_development_v1: 6,
    tier_b_monthly_elastic_net_sec_clean_v3: 6,
  };
  ...
  // Required inputs validate eligibility; explanation rows contain only the
  // model's non-zero coefficients and therefore determine the displayed rank.
  ...
  return <AppShell current="">
    ...
    {eligibleScore ? <>
      ...
      <section className="content-section"><div className="section-heading score-input-heading"><div><p className="eyebrow">WHY THIS SCORE</p><h2>What influenced it</h2></div><dl className="score-input-summary" aria-label="Model input summary"><div><dt>Active weights</dt><dd>{explainedInputs.length}</dd><small>determine rank</small></div>{requiredInputCount !== undefined && <div><dt>Required inputs</dt><dd>{requiredInputCount}</dd><small>validate score</small></div>}</dl></div>{explainedInputs.length ? <><p className="evidence-intro">{requiredInputCount !== undefined ? `${requiredInputCount} dated inputs are checked. ${explainedInputs.length} active model weights determine this company’s rank; the others must still be present, or the score is withheld.` : "The model compares each active input with companies in the same sector. These are the clearest current influences, not a prediction or trade instruction."}</p><div className="evidence-groups"><section className="evidence-group" aria-labelledby="supports-score"><div className="evidence-group-heading"><h3 id="supports-score">Supports the score</h3><span>Strongest positive inputs</span></div>{supports.length ? <ul className="evidence-list">{supports.map((item) => <li key={`${item.featureKey}-${item.featureVersion}`}><div><strong>{evidenceName(item)}</strong><span>{formatPercentile(item.percentile)} among {item.sectorCode} peers</span></div><p>{evidenceCopy(item)}</p></li>)}</ul> : <p className="evidence-empty">No active input is currently lifting this score.</p>}</section><section className="evidence-group" aria-labelledby="headwinds-score"><div className="evidence-group-heading"><h3 id="headwinds-score">Worth watching</h3><span>Inputs holding the score back</span></div>{headwinds.length ? <ul className="evidence-list">{headwinds.map((item) => <li key={`${item.featureKey}-${item.featureVersion}`}><div><strong>{evidenceName(item)}</strong><span>{formatPercentile(item.percentile)} among {item.sectorCode} peers</span></div><p>{evidenceCopy(item)}</p></li>)}</ul> : <p className="evidence-empty">No active input is currently holding this score back.</p>}</section></div></> : <p className="empty-inline">Feature explanations are not available for this dated score.</p>}</section>
