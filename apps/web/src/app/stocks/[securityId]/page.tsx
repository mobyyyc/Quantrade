...
export default async function StockDetailPage({ params, searchParams }: { params: Promise<{ securityId: string }>; searchParams: Promise<{ date?: string; from?: string }> }) {
  ...
  return <AppShell current="">
    ...
    {marketSnapshot && <section className="stock-overview" aria-label="At a glance"><div className="stock-overview-item"><span>Latest close</span><strong>{formatUsdPrice(marketSnapshot.closePrice)}</strong>{priceChange ? <small className={priceChange.direction === "positive" ? "positive-change" : "negative-change"}>{priceChange.percent} since prior close</small> : <small>{formatResearchDate(marketSnapshot.sessionDate)} close</small>}</div><div className="stock-overview-item"><span>Research score</span><strong>{eligibleScore ? `${formatScore(eligibleScore.score)}/100` : "Unavailable"}</strong><small>{eligibleScore ? `Published ${formatResearchDate(eligibleScore.scoreDate)}` : score ? `Checked ${formatResearchDate(score.scoreDate)}` : "Awaiting eligible inputs"}</small></div><div className="stock-overview-item"><span>Market value</span><strong>{marketSnapshot.marketValue ? formatCompactUsd(marketSnapshot.marketValue) : "Unavailable"}</strong><small>{marketSnapshot.sharesReportedFor ? `Latest reported shares, ${formatResearchDate(marketSnapshot.sharesReportedFor)}` : "Reported share count unavailable"}</small></div><div className="stock-overview-item"><span>Market data</span><strong>{formatResearchDate(marketSnapshot.sessionDate)}</strong><small>Latest regular-session close</small></div></section>}
    {identity && <PriceChart points={priceHistory} ticker={identity.ticker} />}
    {eligibleScore ? <>
      <section className="detail-score"><div><p className="eyebrow">RESEARCH SCORE</p><p className="anchor-score">{formatScore(eligibleScore.score)}<span>/100</span></p><p className="quiet-copy">Rank {eligibleScore.rank ?? "Unavailable"} · {eligibleScore.signal} · Tier {eligibleScore.dataCapabilityTier}</p></div><div className="detail-context"><span>Data cutoff</span><strong>{formatPublicationTime(eligibleScore.dataCutoffAt)}</strong><span>Published</span><strong>{formatPublicationTime(eligibleScore.publishedAt)}</strong><span>Model</span><strong>{eligibleScore.modelVersion}</strong></div></section>
