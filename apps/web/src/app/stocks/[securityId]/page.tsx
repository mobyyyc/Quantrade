...
export default async function StockDetailPage({ params, searchParams }: { params: Promise<{ securityId: string }>; searchParams: Promise<{ date?: string; from?: string }> }) {
  ...
  // The active private-beta model is registered with six required percentile inputs.
  // Elastic-net regularization may retain fewer non-zero weights for ranking.
  const requiredInputCount = score?.modelVersion === "tier_b_regularized_linear_development_v1" ? 6 : undefined;
