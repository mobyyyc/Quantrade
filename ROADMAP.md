...
- [ ] P12.9: align operations with the approved 9:45 p.m. daily backup and 10:15 p.m. weekday update schedule. Repository implementation and tests are ready: both actions are hidden and Codex-independent; backup catch-up remains safe at login; the daily task adds a guarded, logged same-evening trigger that never backdates a missed score. Final completion requires one elevated run of `scripts/install-operations-schedule.ps1`, followed by inspection of the installed triggers.
