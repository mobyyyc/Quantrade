...
- [ ] P15.5: audit secrets and configuration, rotate previously exposed provider credentials, and prevent secrets from entering source control or logs. Repository/history scanning and local ACL tooling are implemented; the audit found no real credential values in Git. Completion remains blocked on user-side revocation/replacement of the exposed Alpaca key pair and rotation of the local PostgreSQL password without sharing either value.
